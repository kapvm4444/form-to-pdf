...
export default function Form() {
    ...
    function handleSubmit(print){
        documentize({
            name, age, gender, mobile, village, city, state, diagnosis, treatment, medicine
        })
    }


    return (
        <form className="max-w-sm mx-auto mt-8" onSubmit={(e => e.preventDefault())}>

            {/*Patient Name Field*/}
            <TextField placeholder={"i.e. John Doe"} value={name} onChange={el => setName(el.target.value)}>
                Patient Name
            </TextField>
